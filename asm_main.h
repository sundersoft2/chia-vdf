...
ASM_FUNC(integer_gcd,
    ...
    bool integer_gcd_asm(simd_integer& a, simd_integer& b, simd_integer& v0, int64& v0_sign) {
        ...
        //print( "simd gcd:", end_time-start_time);
    ...
    simd_integer_asm transfer_padded(simd_integer_asm targ, mpz_t a, int num_limbs) {
    ...
    void transfer(mpz_t a, simd_integer_asm targ, int num_limbs) {
    ...
    void integer_gcd_asm(mpz_t g, mpz_t s, mpz_t t, mpz_t a, mpz_t b) {
        assert(t==nullptr);
        ...
        if (mpz_sizeinbase(a, 2)<=max_size && mpz_sizeinbase(b, 2)<=max_size) {
...
ASM_FUNC(integer_reduce,
    ...
    bool integer_reduce_asm(simd_integer& a, simd_integer& b, simd_integer& c, int64& b_sign) {
        ...
        //print( "simd reduce:", end_time-start_time);
    ...
    bool integer_reduce_asm(mpz_t a, mpz_t b, mpz_t c) {
        ...
        if (
            mpz_sizeinbase(a, 2)<=max_size &&
            mpz_sizeinbase(b, 2)<=max_size &&
            mpz_sizeinbase(c, 2)<=max_size
        ) {
            //arguments are backwards because the first input is smaller than the second
            simd_integer_asm a_int=transfer_padded(global_asm_ints[1], a, num_limbs);
            simd_integer_asm b_int=transfer_padded(global_asm_ints[2], b, num_limbs);
            simd_integer_asm c_int=transfer_padded(global_asm_ints[3], c, num_limbs);
